...
export default function TemasScreen({ navigation }) {
  ...
  // Nueva función para resetear temas
  const resetTemas = async () => {
    const snapshot = await getDocs(collection(db, 'temas'));
    const temas = await Promise.all(
      snapshot.docs.map(async (doc) => {
        const temaData = doc.data();
    ...
    <View style={[styles.container, { backgroundColor: darkModeEnabled ? '#121212' : '#f8f8f8' }]}>
      ...
      <Modal animationType="slide" transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={[styles.modalView, { backgroundColor: darkModeEnabled ? '#333' : 'white' }]}>
          ...
          {selectedTema && selectedTema.author === user?.uid && (
            <>
              <TouchableOpacity
                style={styles.button}
                onPress={() => {
      ...
      <View style={styles.footerButtons}>
        ...
        <Button title="Añadir nuevo tema" onPress={() => navigation.navigate('CrearTemaScreen')} />
        <TouchableOpacity onPress={() => navigation.navigate('UserSettings')} style={styles.settingsButton}>
          <Icon name="settings-outline" size={30} color={darkModeEnabled ? '#fff' : '#333'} />
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  resetButton: {
    backgroundColor: '#FF6347', // Color de fondo para el botón "Resetear Temas"
    borderRadius: 10,
    padding: 10,
    elevation: 2,
  },
